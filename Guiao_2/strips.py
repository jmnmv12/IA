...
class STRIPS(SearchDomain):
    ...
    def result(self, state, action):
        ...
        print(f"Statess: {self.actions(state)}")
        if action.__repr__() not in [i.__repr__() for i in self.actions(state)]:
            #print("None")
            return None
    ...
    def satisfies(self, state, goal):
        ...
        return all(e in state for e in goal)   
